...
import { createTabCountQuery } from '../hooks/create-tab-count-query';
...
function Main() {
  ...
  const tabCountState = createTabCountQuery();
  ...
  return (
    <div class="w-full">
      <Header
        tabs={tabState().tabs.map(tab => ({ ...tab, count: tabCountState[tab.key] }))}
